  ...
  return (
    <div className="px-6 pt-10 pb-20 font-teko flex flex-col gap-2 items-center w-full text-indigo-950">
      <SectionHeader
        ...
        bgColor="bg-primary"
      />
      <p className=" flex flex-wrap lg:text-5xl sm:text-4xl text-3xl font-semibold uppercase text-center mt-2 max-sm:mx-2">
        Choose a Perfect Plan For You{" "}
      </p>
      <div className="flex justify-center items-center flex-wrap gap-8 mt-5 ">
        {plans.map((item, index) => (
          <div
            className="   flex  flex-col  gap-2 p-5  justify-center  border shadow-lg  bg-white font-barlow "
            key={index}
          >
            ...
            <button
              className=" relative bg-primary  text-white py-2 mt-4 rounded font-medium overflow-hidden   group "
              onClick={() => paymentRazorPay(item.planType)}
            >
